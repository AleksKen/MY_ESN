...
# Определение резервуарной сети (Echo State Network) с наблюдателем (readout layer)
class EchoStateNetworkWithObserver(nn.Module):
    def __init__(self, input_size, reservoir_size, output_size):
        super(EchoStateNetworkWithObserver, self).__init__()
        ...
        self.readout = nn.Linear(reservoir_size, output_size)  # Линейный слой наблюдения (readout)
    ...
    def forward(self, input_data):
        ...
        output = torch.tanh(self.readout(reservoir_input))  # Применяем нелинейность к выходу наблюдателя
